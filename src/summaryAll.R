...
idxS <- filter(d, flag %in% flags[-1:-3]) %>%
    .$taxNameSpecies %>%
    unique()
...
p +
    geom_point(shape = 22) +
    scale_fill_manual(name = "Flag", values = idxC) +
    scale_colour_manual(name = "Flag", values = idxC) +
    scale_size_continuous("Number of reads", range = c(1, 4), trans = "log10") +
    xlab("Contig") +
    ylab("Sample") +
...
##
d2 <- filter(d, flag == "damage_0.1;editDistAvg_1.5;editDistDecay_5", contigId != "genome") %>%
    mutate(flag1 = 1L) %>%
    select(sampleId, contigId, flag1) %>%
    pivot_wider(names_from = contigId, values_from = flag1) %>%
...
d4 <- cmdscale(d3) %>%
    as_tibble() %>%
    mutate(sampleId = rownames(d2))

pdf(paste("plots/", prefix, ".mds.pdf", sep = ""), width = 8, height = 8)
p <- ggplot(d4, aes(x = V1, y = V2))
p +
    geom_point(shape = 21, color = "steelblue", fill = "steelblue", alpha = 0.9) +
    geom_text_repel(aes(label = sampleId), size = 1, segment.size = 0.25) +
    xlab("MDS 1") +
    ylab("MDS2") +
    theme_bw() +
    theme(panel.grid.major = element_line(linetype = "dotted", size = 0.25), panel.grid.minor = element_blank(), strip.background = element_blank())
dev.off()
