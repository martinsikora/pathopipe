...
d <- map_dfr(files, ~ {
    r <- read_tsv(
        file = .x,
        col_types = "ccccccdddddddddddddddddddddddddddddddddc",
        na = c("", "NA", "NaN")
    )
    r
})
...
d3 <- d3 %>%
    mutate(
        contigId = factor(contigId, levels = unique(d32$contigId)),
...
walk(unique(d3$genusId), ~ {
    ...
    pdf(
        paste("plots/targets_priority/", .x, ".",
            prefix, ".targets_priority.matrix.pdf",
            sep = ""
        ),
        width = w,
        height = h
    ...
    print(p +
        ...
        xlab("Contig") +
        facet_nested(
            . ~ genusId + taxNameSpecies + assemblyId,
            scales = "free_x",
            space = "free",
            nest_line = TRUE
        ) +
...
pdf(
    paste("plots/targets_priority/", prefix,
        ".targets_priority.pdf",
        sep = ""
    ),
    width = 9,
    height = 7
)
walk(unique(d3$sampleId), ~ {
    d4 <- d1 %>%
        filter(
            sampleId == .x,
            contigId == "genome"
        ...
        mutate(
            isTop = krakenKmerRank < 3 & !is.na(aniRank100) &
                !is.na(krakenKmerRank) & covPosRelEntropy1000 >= 0.9,
            ani1 = case_when(
                ani >= 0.95 ~ ani,
                TRUE ~ NaN
            )
    ...
    d42 <- d41 %>%
        filter(flag == "damage_0.05;ani_0.97;covPosRelEntropy1000_0.9;krakenKmerRank_1")
    ...
    print(p +
        ...
        scale_size_continuous(
            ...
            range = c(1, 5)) +
        scale_alpha_manual(
            name = "Kraken kmer rank < 3\ncovPRelEntropy1000 > 0.9", 
            values = c(0.2, 1)) +
        scale_x_continuous(
            breaks = xBreaks, 
            trans = "log10", 
