...
d <- foreach(f1 = bamfiles) %do% {
    ...
    if(nrow(r2) == 0){
        ...
        r3 <- gsub(".*\\/", "", f1) %>%
            strsplit("\\.") %>%
            unlist()
        assemblyId <- paste(r3[2:(length(r3) - 4)], collapse = ".")
        r4 <- count(r2, value) %>%
            rename(editDist = value) %>%
            mutate(p = n / sum(n), assemblyId = assemblyId, taxNameSpecies = seqInfo$taxNameSpecies[match(assemblyId, seqInfo$assemblyId)]) %>%
...
if(nrow(d) == 0){
    ...
    print(p +
          ...
          scale_shape_manual(values = idxS) + scale_colour_manual(values = idxC) + th + guides(shape = guide_legend(ncol = 4, override.aes = list(size = 0.75, stroke = 0.1))))
    print(p +
          geom_line(size = 0.25, show.legend = FALSE) +
          geom_point(size = 1) +
          xlab("Edit distance") +
